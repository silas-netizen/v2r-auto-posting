...
- 로그인은 영구 프로필 쿠키로 유지하고 상태 확인 시 keep-alive 요청을 보내지 않음
- 등록 버튼을 누르기 전에 `uncertain` 체크포인트를 먼저 저장
- 등록·수정 완료 URL을 확인하는 즉시 해당 행을 완료 이력으로 갱신
- 네트워크나 창이 끊긴 행은 자동 재발행하지 않고 수동 확인 대상으로 보류
