...
class SearchVolumeApp(AutomationApp):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(
            outer,
            text="구글 시트에서 검색량이 비어 있는 키워드만 채웁니다. 0은 이미 채운 값입니다. 띄어쓰기는 자동완성 첫 항목이 같은 글자면 따르고, 다른 단어가 붙으면 무시합니다. 없으면 통합검색 첫 카페 글 제목을 따릅니다. 통합검색 주소와 한국 시각도 같이 넣습니다. 값은 이름 상자로 칸을 고른 뒤 그 칸에 바로 덮어씁니다. 수식 입력줄에 글자가 안 보여도, 시트보내기에 값이 남으면 성공입니다. 이미 같은 값이면 그대로 두고 오류를 내지 않습니다. 붙여넣기 설정 창이 뜨면 취소합니다. 설치할 필요 없습니다.",
