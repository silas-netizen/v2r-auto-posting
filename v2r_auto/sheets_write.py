...
def build_join_batch_update(plan, sheet_numeric_id: int) -> dict[str, Any]:
    """Replace only cafe columns. Other columns stay untouched."""
    requests: list[dict[str, Any]] = []
    for group in plan.contiguous_cafe_groups():
        start_column = plan.column_index(group[0])
        rows = []
        for row in plan.rows:
            rows.append(
                {
                    "values": [
                        {"userEnteredValue": {"stringValue": row.get(header) or ""}}
                        for header in group
                    ]
                }
            )
        requests.append(
            {
                "updateCells": {
                    "range": {
                        ...
                        "endColumnIndex": start_column + len(group),
                    },
                    "rows": rows,
